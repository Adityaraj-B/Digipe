...
class _InsuranceDetailScreenState extends State<InsuranceDetailScreen> {
        ...
        if (mounted) {
          setState(() {
            ...
            if (_selectedState != null &&
                indiaLocationData.containsKey(_selectedState!)) {
              _availableCities =
                  List<String>.from(indiaLocationData[_selectedState]!)..sort();
            }
  ...
  Future<void> _saveFormData() async {
    try {
      ...
      await _storage.write(
          key: 'insurance_form_cache', value: jsonEncode(data));
      ...
      try {
        ...
        if (authState is Authenticated) {
          ...
          final shouldUpdate =
              (current.house == null || current.house!.isEmpty) ||
          ...
          if (shouldUpdate) {
            context.read<AuthBloc>().add(AuthUpdateProfileRequested(
                  ...
                  house: _houseController.text.trim().isEmpty
                      ? null
  ...
  Future<void> _loadFieldDefinitions() async {
    ...
    if (productBloc.state is ProductLoaded) {
      ...
      try {
        final defs =
            await context.read<ApiService>().getProductFields(productId);
    ...
    if (result != null && result.files.single.path != null) {
      ...
      try {
        final doc = await context
            .read<ApiService>()
            .uploadDocumentFull(result.files.single.path!);
        if (!mounted) return;
        setState(() {
  ...
  Future<void> _pickPhoto(int index) async {
    ...
    if (picked != null) {
      ...
      try {
        final doc =
            await context.read<ApiService>().uploadDocumentFull(picked.path);
        if (!mounted) return;
                    ...
                    Expanded(
                      child: SingleChildScrollView(
                        ...
                        child: Column(
                          ...
                          children: [
                            _buildOrderSummary(
                                plan, productState.selectedProduct.name),
                            ...
                            const SizedBox(height: 16),
                            _buildSection(4, 'Upload Documents',
        ...
        children: [
          Row(children: [
            CircleAvatar(
              radius: 14,
              backgroundColor:
                  isDark ? const Color(0xFFF5A623) : const Color(0xFF2C2C2E),
              child: Text('$number',
                  style: TextStyle(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildSummaryRow('Sum Insured', '₹${plan.coverageAmount}',
              isValueBold: true),
          _buildSummaryRow('Duration', '${plan.durationMonths ~/ 12} Year',
              isValueBold: true),
          ...
          _buildSummaryRow('GST (${_gstPercentage.toStringAsFixed(0)}%)',
  ...
  Widget _buildInstallationAddressContent() => Column(children: [
        ...
        Row(children: [
          Expanded(
              child: _buildTextField('PIN Code', 'Enter PIN code',
    ...
    return Column(
      ...
      children: [
        ...
        DropdownButtonFormField<String>(
          key: ValueKey(
              '${label}_${uniqueItems.length}_${effectiveValue ?? 'none'}'),
          ...
          decoration: InputDecoration(
            filled: true,
            fillColor:
                isDark ? const Color(0xFF2C2C2E) : const Color(0xFFF7F7F8),
            contentPadding:
  ...
  Widget _buildSolarInstallationDetailsContent() => Column(children: [
        Row(children: [
          ...
          const SizedBox(width: 12),
          Expanded(
              child: _buildDateField('Installation Date', 'e.g. 15-06-2026')),
        ]),
        const SizedBox(height: 12),
        Row(children: [
          Expanded(
              child: _buildTextField('Brand / Manufacturer', 'e.g. Tata Solar',
                  controller: _brandController)),
          const SizedBox(width: 12),
          Expanded(
              child: _buildTextField(
                  'Installer / Contractor', 'e.g. Green Energy',
      ...
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _invoiceName == null
            ...
            : _buildFileChip(
                ...
                name: _invoiceName!,
                onRemove: () => setState(() => _invoiceName = null)),
        ...
        _videoName == null
            ...
            : _buildFileChip(
                ...
                name: _videoName!,
                onRemove: () => setState(() => _videoName = null)),
        const SizedBox(height: 20),
        _buildInlineCheckbox(
            value: _confirmNoDamage,
            onChanged: (v) => setState(() => _confirmNoDamage = v ?? false),
            label:
                'I confirm that the product has no damage and is not pre-repaired.'),
      ]);

  Widget _buildUploadLabel(String label) => Text(label,
      ...
      Container(
        ...
        child: Row(children: [
          ...
          GestureDetector(
              onTap: onRemove,
              child:
                  const Icon(Icons.close, size: 18, color: Color(0xFF8E8E93))),
        ]),
      );
      ...
      Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Expanded(
          child: Padding(
            padding: const EdgeInsets.only(top: 2.0),
            child: Text(
                'I hereby declare that the details provided above are true.',
                style: TextStyle(
                    fontFamily: 'Poppins',
                    fontSize: 12,
                    color: Theme.of(context).colorScheme.onSurface)),
          ...
          child: ElevatedButton(
            ...
            onPressed: (_confirmNoDamage && _confirmDeclaration)
                ? () async {
                    ...
                    final nameErr = InputValidator.required(
                        _nameController.text, 'Full Name');
                    final phoneErr =
                        InputValidator.phone(_phoneController.text);
                    final emailErr =
                        InputValidator.email(_emailController.text);
                    final houseErr = InputValidator.required(
                        _houseController.text, 'House / Shop No.');
                    final areaErr = InputValidator.required(
                        _areaController.text, 'Area / Locality');
                    final districtErr = InputValidator.required(
                        _districtController.text, 'District');
                    final pinErr = InputValidator.required(
                        _pinController.text, 'PIN Code');
                    final capacityErr = InputValidator.required(
                        _capacityController.text, 'Capacity (kW) / Size');
                    final brandErr = InputValidator.required(
                        _brandController.text, 'Brand / Manufacturer');
                    final installerErr = InputValidator.required(
                        _installerController.text, 'Installer / Contractor');

                    final stateErr =
                        _selectedState == null ? 'Please select a State' : null;
                    final cityErr =
                        _selectedCity == null ? 'Please select a City' : null;
                    final dateErr = _installationDate == null
                        ? 'Please select Installation Date'
                        : null;

                    final invoiceErr = _invoiceUrl == null
                        ? 'Please upload the Invoice File'
                        : null;
                    final photoErr = !_photoUrls.any((url) => url != null)
                        ? 'Please upload at least 1 Site Installation Photo'
                        : null;

                    final firstError = nameErr ??
                        phoneErr ??
                        emailErr ??
                        houseErr ??
                        areaErr ??
                    ...
                    final rawValues = [
                      ...
                      {
                        'name': 'district',
                        'val': _districtController.text.trim()
                      },
                      {'name': 'state', 'val': _stateController.text.trim()},
                      {'name': 'pinCode', 'val': _pinController.text.trim()},
                      {
                        'name': 'capacity',
                        'val': _capacityController.text.trim()
                      },
                      {'name': 'brand', 'val': _brandController.text.trim()},
                      {
                        'name': 'installationDate',
                        'val': _installationDate?.toIso8601String() ?? ''
                      },
                      {
                        'name': 'installerName',
                        'val': _installerController.text.trim()
                      },
                      ...
                      {
                        'name': 'sitePhoto1Name',
                        'val': _photoUrls[0] != null ? 'Site_Photo_1.jpg' : ''
                      },
                      {'name': 'sitePhoto2Url', 'val': _photoUrls[1] ?? ''},
                      {
                        'name': 'sitePhoto2Name',
                        'val': _photoUrls[1] != null ? 'Site_Photo_2.jpg' : ''
                      },
                      {'name': 'sitePhoto3Url', 'val': _photoUrls[2] ?? ''},
                      {
                        'name': 'sitePhoto3Name',
                        'val': _photoUrls[2] != null ? 'Site_Photo_3.jpg' : ''
                      },
                      {'name': 'sitePhoto4Url', 'val': _photoUrls[3] ?? ''},
                      {
                        'name': 'sitePhoto4Name',
                        'val': _photoUrls[3] != null ? 'Site_Photo_4.jpg' : ''
                      },
                    ...
                    for (var input in rawValues) {
                      if (input['val'] == null ||
                          input['val'].toString().trim().isEmpty) continue;

                      final def = _fieldDefinitions
                          .where((d) => d.fieldName == input['name'])
                          .firstOrNull;
                    ...
                    try {
                      ...
                      if (alreadyExists != null) {
                        ...
                        dev.log(
                            'Existing application found, skipping duplicate creation.');
                      } else {
                        try {
                          ...
                          if (e.response?.statusCode == 409) {
                            ...
                            if (application == null)
                              throw Exception(
                                  'Conflict detected but existing application not found.');
                          } else {
                            rethrow;
                          }
                        }
                        ...
                        Navigator.pushReplacement(
                          ...
                          MaterialPageRoute(
                            builder: (_) => OrderTrackingScreen(
                              orderId: application!['applicationNumber'] ??
                                  application['_id'] ??
                                  application['id'],
                            ),
                          ),
                        ...
                        onPressed: () async {
                          ...
                          try {
                            ...
                            NotificationService.showNotification(
                              ...
                              body:
                                  'Your insurance application is now being processed.',
                            );
    ...
    try {
      ...
      if (e.response?.statusCode == 400 &&
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  message.isNotEmpty ? message : 'Order creation failed.')),
        );
      }
    } catch (e) {
      if (!mounted) return;
